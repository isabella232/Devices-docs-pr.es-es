...
>[!NOTE]
>Surface Hub usará la entrada de vídeo que seleccione hasta que se realice una nueva conexión, se interrumpa la conexión existente o se cierre la aplicación Conectar.
...
| Encienda el Surface Hub desde un equipo externo Windows 10 o Windows 11, desactivando el equipo incrustado del Surface Hub. Las cámaras, los micrófonos, los altavoces y otros periféricos se envían al equipo externo, además de las entradas de lápiz y táctiles. | [Modo de equipo de reemplazo](#replacement-pc-mode) |

## <a name="guest-mode"></a>Modo Invitado

El modo Invitado usa una conexión por cable para que los usuarios puedan mostrar el contenido de sus dispositivos en Surface Hub. Si el dispositivo de origen es un dispositivo Windows, dicho dispositivo también puede proporcionar touchback e inkback. El equipo interno de Surface Hub toma el audio y el vídeo del dispositivo conectado y los presenta en Surface Hub. Si Surface Hub encuentra una señal de protección de contenido digital (HDCP) High-Bandwidth, el origen se mostrará como una imagen negra. Para mostrar tu contenido sin infringir los requisitos de HDCP, usa el teclado del lado derecho de Surface Hub para elegir directamente el origen externo.
...
| ----------------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Puerto de pantalla 1.1a | Entrada de vídeo | Entrada de invitado 1     | - Admite la visualización de entrada de invitado simultánea con la entrada de invitado n.º 2 y la entrada de invitado n.º 3 (una resolución completa, dos miniaturas).<br>- Compatible con HDCP en modo de omisión<br>- Touchback habilitado |
| HDMI 1.4          | Entrada de vídeo | Entrada de invitado 2     | - Admite la visualización simultánea de la entrada de invitado con la entrada de invitado n.º 1 y la entrada de invitado n.º 3 (una resolución completa, dos miniaturas).<br>- Compatible con HDCP en modo de omisión<br>- Touchback habilitado |
| VGA               | Entrada de vídeo | Entrada de invitado 3     | - Admite la visualización simultánea de la entrada de invitado con la entrada de invitado n.º 1 y la entrada de invitado n.º 2 (una resolución completa, dos miniaturas).<br>- Compatible con HDCP en modo de omisión<br>- Touchback habilitado |
| Conector de 3,5mm       | Entrada de audio | Entrada de audio analógico | - Ingerir en Surface Hub PC, normalmente con la entrada de vídeo VGA.                                                                                                                   |
| USB 2.0, tipo B   | Salida USB     | Touchback          | - Proporciona acceso al mouse, el teclado y el lápiz táctil de los dispositivos de entrada HID al equipo invitado.                                                                               |
...
#### <a name="human-interface-devices"></a>Dispositivos de interfaz humana (HID)
...
#### <a name="mice-and-other-pointing-devices"></a>Mouse y otros dispositivos señaladores

- Mouse compatible con HID

#### <a name="universal-serial-bus-controllers"></a>Controladoras de bus serie universal
...
El audio de origen lo proporcionan los cables de DisplayPort y HDMI. Si tienes que usar VGA, Surface Hub dispone de un puerto de entrada de audio que usa un conector de 3,5mm. Surface Hub también usa un cable USB que proporciona Touchback y Inkback desde el Surface Hub a dispositivos Windows 10 o Windows 11 compatibles. El cable USB puede usarse con cualquier entrada de vídeo que ya esté conectada con un cable.
...
Puede ejecutar Surface Hub en modo pc de reemplazo con versiones de 64 bits de Windows 10 o Windows 11 Home, Windows 10 o Windows 11 Pro, y Windows 10 o Windows 11 Empresas. Puedes descargar el [paquete de controladores de PC de reemplazo para Surface Hub](https://www.microsoft.com/download/details.aspx?id=52210) desde el Centro de descarga de Microsoft. Te recomendamos que instales a estos controladores en cualquier equipo que vayas a usar como equipo de reemplazo.

### <a name="hardware-requirements"></a>Requisitos de hardware

Surface Hub es compatible con una gran variedad de hardware. Elige la confirmación del procesador y la memoria para el equipo de reemplazo, de forma que admita los programas que usarás. El hardware del equipo de reemplazo debe admitir versiones de 64 bits de Windows 10 o Windows 11.
...
### <a name="replacement-pc-ports"></a>Puertos de equipo de reemplazo
...
| -------------------- | ----------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Vídeo del equipo             | Entrada de vídeo | DP 1.2         | - Pantalla completa de 1080p a 120 Hz, además de audio<br>- Compatible con HDCP                                                           |
| Periféricos internos | Salida USB  | USB 2.0 tipo B | - Toque<br>- Lápiz<br>- Altavoces<br>- Micrófono<br>- Cámaras<br>- Sensor NFC<br>- Sensor de luz ambiente<br>- Sensor infrarrojo pasivo |
| Concentrador USB              | Salida USB  | USB 2.0 tipo B | - Debajo de los puertos USB                                                                                                             |
...
| Descripción          | Tipo        | Interfaz      | Detalles                                                                                                                            |
| -------------------- | ----------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Vídeo del equipo             | Entrada de vídeo | DP 1.2 (2x)    | - Pantalla completa de 2160p a 120 Hz, más audio<br>- Compatible con HDCP                                                           |
| Periféricos internos | Salida USB  | USB 2.0 tipo B | - Toque<br>- Lápiz<br>- Altavoces<br>- Micrófono<br>- Cámaras<br>- Sensor NFC<br>- Sensor de luz ambiente<br>- Sensor infrarrojo pasivo |
| Concentrador USB              | Salida USB  | USB 2.0 tipo B | - Debajo de los puertos USB                                                                                                             |

### <a name="replacement-pc-setup-instructions"></a>Instrucciones de configuración para el equipo de reemplazo

#### <a name="to-use-replacement-pc-mode"></a>Para usar el modo Equipo de reemplazo
...
3. Conecta los cables de los puertos del equipo de reemplazo de Surface Hub al equipo de reemplazo. Generalmente estos puertos se encuentra cubiertos por una tapa de plástico extraíble.
    ...
    Surface Hub de 55": conecte un cable DisplayPort y dos cables USB.

    Surface Hub de 84": conecte dos cables DisplayPort y dos cables USB.
...
#### <a name="to-switch-back-to-internal-pc"></a>Para volver a su PC interno
...
### <a name="video-out-ports"></a>Puertos de salida de vídeo

Puerto de salida de vídeo en Surface Hub 55"

![Ilustración del puerto de salida de vídeo en Surface Hub de 55".](images/video-out-55.png)

Puerto de salida de vídeo en Surface Hub 84"

![Ilustración del puerto de salida de vídeo Surface Hub de 84".](images/video-out-84.png)

| Descripción         | Tipo         | Interfaz    | Funcionalidades                                                                                                                                                                                                               |
| ------------------- | ------------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reflejo de salida de vídeo | Salida de vídeo | Salida de vídeo | - Admite la conexión a un monitor DisplayPort estándar (solo admite un vínculo x4 que muestra una resolución de 1080p60 a 24bpp)<br>- Admite el uso con monitores HDMI (compatible con 1080p60) mediante un adaptador DisplayPort-to-HDMI |
